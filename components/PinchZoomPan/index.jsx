...
const RESET_ANIMATION_SPEED = 1;
const INITIAL_X = 0;
const INITIAL_Y = 0;
const INITIAL_SCALE = 1;

const settle = (val, target, range) => {
  const lowerRange = val > target - range &&
    val < target;
  const upperRange = val < target + range &&
    val > target;
...
const checkBetween = (m1, m2, value) => (m1 < m2 ?
  between(m1, m2, value) : between(m2, m1, value)
);
...
class PinchZoomPan extends Component {
  ...
  zoom(scale, midpoint) {
    ...
    if (this.props.isZoomed === false && scale !== 1) {
      this.props.zoomIn();
    } else if (this.props.isZoomed === true && scale === 1) {
      this.props.zoomOut();
    }
  }
